...
def add_faq(faq: FAQRequest):
    ...
    with sqlite3.connect('faqs.db') as conn:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO faqs (question, answer, question_embedding) VALUES (?, ?, ?)",
...
def query_faq(query: QueryRequest):
    ...
    for faq_id, question, answer, question_embedding in faqs:
        stored_embedding = torch.tensor(list(struct.unpack('%sf' % (len(question_embedding) // 4), question_embedding)))
    ...
    if best_match[3] > 0.7:
        return {
            ...
            "similar_questions": [{"question": faq[1], "similarity": faq[3]} for faq in similarities[1:4]]
        }
    else:
        return {
            "message": "No exact match found. Here are some similar questions.",
            "similar_questions": [{"question": faq[1], "answer": faq[2], "similarity": faq[3]} for faq in similarities[:3]]
        }
